...
namespace ForgottenMemories.NPCs.GhastlyEnt
{
    public class WormHead : ModNPC
    {
		...
		{
			DisplayName.SetDefault("Woodworm");
		}
		...
		{
			Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/WoodWorm/WoodWormHeadGore"), 1f);
		}
        ...
        {	
            ...
            if (Main.netMode != 1)
            {
                ...
                if (npc.ai[0] == 0)
                {
                    ...
                    int randomWormLength = Main.rand.Next(7, 11);
                    for (int i = 0; i < randomWormLength; ++i)
                    {
                        latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("WormBodyTwo"), npc.whoAmI, 0, latestNPC);
