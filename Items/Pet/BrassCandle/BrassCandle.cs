...
{
	public class BrassCandle : ModProjectile
	{
		...
		{
			projectile.ai[0]++;
			if(projectile.ai[0] >= (Main.rand.Next(-10, 11) + 120))
			{
				int p = Projectile.NewProjectile(projectile.Center, Vector2.Zero, 400 + Main.rand.Next(3), Main.rand.Next(6) + 7, 1f, projectile.owner, 0, 0);
			...
			if (Main.player[projectile.owner].dead)
				projectile.Kill();
			if (Main.myPlayer == projectile.owner && modPlayer.brassCandle)
			...
			Lighting.AddLight(projectile.position, 1f, 0.7f, 0f);
